...
void recurDiagonalTreeSum(Node *root, int vd, map<int, int> &diagonalSums){
    if(root == NULL){
        return;
    }
    diagonalSums[vd] += root -> data;
    
    recurDiagonalTreeSum(root -> left, vd + 1, diagonalSums);
    
    recurDiagonalTreeSum(root -> right, vd, diagonalSums);
}
...
vector <int> diagonalSum(Node* root) {
    ...
    vector<int> vec;
    recurDiagonalTreeSum(root, 0, diagonalSums);
    for(auto it : diagonalSums){
